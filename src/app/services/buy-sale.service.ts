...
export class BuySaleService {
  ...
  checkForSave(record: BuySaleMainModel): Promise<string> {
    return new Promise((resolve, reject) => {
      if (record.data.currencyPrimaryKey === null || record.data.currencyPrimaryKey === '-1') {
  ...
  checkFields(model: BuySaleModel): BuySaleModel {
    ...
    if (model.currencyPrimaryKey === undefined) { model.currencyPrimaryKey = cleanModel.currencyPrimaryKey; }
    ...
    if (model.unitAmount === undefined) { model.unitAmount = cleanModel.unitAmount; }
    if (model.unitValue === undefined) { model.unitValue = cleanModel.unitValue; }
    if (model.totalAmount === undefined) { model.totalAmount = cleanModel.totalAmount; }
    ...
    if (model.recordDate === undefined) { model.recordDate = cleanModel.insertDate; }
    return model;
  }
